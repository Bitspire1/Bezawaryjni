        ...
        <section id="faq" className="bg-[#0b0b0b] py-16 text-white">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <h2
                    ...
                    data-tina-field={tinaField(data.faq, "heading")}
                >
                    {data.faq.heading}
                </h2>
                <div className="mt-8 divide-y divide-white/10 rounded-lg bg-[#111] ring-1 ring-white/10">
                    {data.faq.items.map((it, index) => (
                        <details key={it.question} className="group open:bg-[#141414]">
                            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3.5 select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300/30 sm:px-5 sm:py-4">
                                <span className="flex min-w-0 items-center gap-3 sm:gap-4">
                                    <span
                                        className="inline-flex h-9 w-9 flex-none items-center justify-center rounded-md bg-yellow-400/10 text-yellow-400 ring-1 ring-yellow-400/30 sm:h-10 sm:w-10"
                                        aria-hidden
                                    >
                                        <Icon name={it.icon} className="h-5 w-5 sm:h-6 sm:w-6" />
                                    </span>
                                    <span
                                        className="wrap-balance leading-snug font-medium break-words text-white/90"
                                        data-tina-field={tinaField(
                                            data.faq.items[index],
                                            "question",
                                        )}
                                    >
                                        {it.question}
                            ...
                            <div
                                className="px-5 pb-5 text-sm leading-relaxed text-white/80"
                                data-tina-field={tinaField(data.faq.items[index], "answer")}
                            >
                                {it.answer}
                            </div>
                        </details>
